...
describe("TRANSACTION class", () => {
  ...
  it("returns date of debit transaction", () => {
    const transaction = new Transaction();
    const mockDebit = new Debit(100, "21/03/2023");
  ...
  it("returns total balance after one credit and one debit transaction", () => {
    const transaction = new Transaction();
    const mockCredit = new Credit(1000, "16/03/2023");
    const mockDebit = new Debit(150, "21/03/2023");

    mockCredit.getCredit.mockImplementation(() => 1000);
    transaction.addCredit(mockCredit);

    mockDebit.getDebit.mockImplementation(() => 150);
    transaction.withdrawDebit(mockDebit);

    expect(transaction.getBalance()).toEqual(850);
  });
});
